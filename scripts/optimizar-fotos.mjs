...
// Bucket a procesar (por defecto product-photos). Uso:
//   node scripts/optimizar-fotos.mjs lotes-fotos
const BUCKET = process.argv[2] || 'product-photos';
...
async function listar(prefix=''){
  const r=await fetch(`${U}/storage/v1/object/list/${BUCKET}`,{method:'POST',
    headers:{...H,'Content-Type':'application/json'},
    body:JSON.stringify({prefix,limit:1000,sortBy:{column:'name',order:'asc'}})});
  return await r.json();
}
async function subir(path,buf){
  const r=await fetch(`${U}/storage/v1/object/${BUCKET}/${path}`,{method:'POST',
...
for(const [i,name] of originales.entries()){
  ...
  try{
    const res=await fetch(`${U}/storage/v1/object/public/${BUCKET}/${name}`);
